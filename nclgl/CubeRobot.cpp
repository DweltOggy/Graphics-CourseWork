...
CubeRobot::CubeRobot(Mesh * mesh) 
{
	...
	head = new SceneNode(mesh, Vector4(0, 1, 0, 1)); // Green !
	head -> SetModelScale(Vector3(5, 5, 5));
	head -> SetTransform(Matrix4::Translation(Vector3(0, 30, 0)));
	body -> AddChild(head);
	
	leftArm = new SceneNode(mesh, Vector4(0, 0, 1, 1)); // Blue !
	leftArm -> SetModelScale(Vector3(3, -18, 3));
	leftArm -> SetTransform(Matrix4::Translation(Vector3(-12, 30, -1)));
	body -> AddChild(leftArm);
	
	rightArm = new SceneNode(mesh, Vector4(0, 0, 1, 1)); // Blue !
...
void CubeRobot::Update(float dt) 
{
	...
	hips->SetTransform(hips->GetTransform() * Matrix4::Rotation(30.0f * dt, Vector3(0, 1, 0)));
